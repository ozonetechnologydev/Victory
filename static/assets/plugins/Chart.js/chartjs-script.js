...
(function(window, document, $, undefined) {
	...
	$(function() {
	   ...
	   if ($('#lineChart').length) {
		...
		if ($('.barChart').length) {
			$(".barChart").toArray().forEach(canvas => {
				var ctx = canvas.getContext('2d');


				var datasets = []

				$(canvas).children().toArray().forEach(element => {
		...
		if ($('.polarChart').length) {
			$(".polarChart").toArray().forEach(canvas => {
				var ctx = canvas.getContext('2d');

				var labels = []
				var values = []
				$(canvas).children().toArray().forEach(element => {
					labels = [...labels, $(element).attr("name")]
		...
		if ($('.pieChart').length) {
			$(".pieChart").toArray().forEach(canvas => {
